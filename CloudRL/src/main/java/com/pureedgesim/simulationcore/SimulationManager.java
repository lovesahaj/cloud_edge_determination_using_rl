...
public class SimulationManager extends SimulationManagerAbstract {
	...
	public void processEvent(SimEvent ev) {
		Task task = (Task) ev.getData();   // it retrieves the data stored in the SimEvent object and casts it to a Task object
		switch (ev.getTag()) {
		case SEND_TO_ORCH:
		...
		case EXECUTE_TASK:
			...
			if (taskFailed(task, 2))
				return;
			getBroker().submitCloudlet(task);  //submits the task cloudlet to the broker
	...
	private void sendFromOrchToDestination(Task task) {
		...
		// handles the scenario when no resources (VM) are available for a task. 
		//It logs the failure, sets the failure reason, and takes appropriate actions.
		if (task.getVm() == Vm.NULL) {
			...
			simLog.taskSentFromOrchToDest(task);      // Recursive Call, if resources are there again initialize and find VM
		}
...
// task.getEdgeDevice().getId(): IDs of the edge device associated with the task is compared with
// with the data center ID where the virtual machine's host is located 
// It compares the orchestrator of the task with the data center object obtained from the virtual machine's host
	...
	private void sendTaskToOrchestrator(Task task) {
		...
		if (SimulationParameters.ENABLE_ORCHESTRATORS) {
			...
			double min = -1; // min is not yet initialized
			int selected = 0;
			double distance;
// calculates the distance between an element in OrchestratorsList and element in task.getEdgeDevice()
			for (int i = 0; i < orchestratorsList.size(); i++) {
				if (orchestratorsList.get(i).getType() != SimulationParameters.TYPES.CLOUD) {  // exclude orchestrators of type "CLOUD"
					distance = orchestratorsList.get(i).getMobilityManager().distanceTo(task.getEdgeDevice()); 
					if (min == -1 || distance < min) {
						min = distance;
						selected = i; 
		//this code snippet updates the min and selected variables if a new minimum value (distance) is found.
						//The selected variable stores the index of the chosen orchestrator.
					}
				}
			}
			...
			task.setOrchestrator(orchestratorsList.get(selected)); // sets the selected orchestrator as orchestrator of task object.
		}
