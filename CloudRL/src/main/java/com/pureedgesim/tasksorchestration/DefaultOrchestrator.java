...
import pruebas.MultiLayerRLManager_python;
import pruebas.MultiLayerRLManager;
import pruebas.RLManager;

public class DefaultOrchestrator extends Orchestrator {
	
	//Adding this
	MultiLayerRLManager_python multiLayerRLManagerPython;
	...
	public DefaultOrchestrator(SimulationManager simulationManager) {
		...
		multiLayerRLManagerPython = new MultiLayerRLManager_python(simLog, simulationManager, orchestrationHistory, vmList, algorithm);
		multiLayerRLManager = new MultiLayerRLManager(simLog, simulationManager, orchestrationHistory, vmList, algorithm);
	}

	/*public int findVM(String[] architecture, Task task) {
	...
	*/
	...
	public int findVM(String[] architecture, Task task) {		
		...
		switch (algorithm) {
			...
			case "RL":
				...
				break;
			case "RL_MULTILAYER", "PYTHON_NEURAL_NET", "RL_MULTILAYER_EMPTY", "RL_MULTILAYER_DISABLED":
	...
	private int onlyType(String[] architecture, Task task, TYPES tipo) {
		int vm = -1;    // to store index to best vm found
		int minTasksCount = -1; // vm with minimum assigned tasks; the number of tasks assigned to that VM
		// get best vm for this task
		for (int i = 0; i < orchestrationHistory.size(); i++) {
			
			if (((DataCenter) vmList.get(i).getHost().getDatacenter()).getType() == tipo) { 
				//it checks if the type of the data center associated with the current VM matches SimulationParameteres.EDGE_DEVICE
		...
		/*if(vm == -1)
			System.out.println("No se env�a a nadie");*/ // Error (enviar a si mismo si eso)
	...
	private int reinforcementLearning(String[] architecture, Task task) {
		...
		// Depending on the decision of the RL algorithm, I launch an offloading policy or another
		if (accion == 0) {			
	...
	private int multilayerreinforcementLearning(String[] architecture, Task task) {
		...
		if (algorithm.equals("PYTHON_NEURAL_NET")) {
			accion = multiLayerRLManagerPython.reinforcementLearning(architecture, task);
		} else {
			accion = multiLayerRLManager.reinforcementLearning(architecture, task);
		}

		// Depending on the decision of the algorithm, RL launches an offloading policy or another
		if (accion == 0) {			
			return local(architecture, task);
		} else if (accion == 1) {			
			String[] architecture2 = { "Mist" };
	...
	public MultiLayerRLManager_python getMultiLayerRLManagerPython() {
		return multiLayerRLManagerPython;
	}
	...
	public void resultsReturned(Task task) {
		...
		} else if (algorithm.equals("PYTHON_NEURAL_NET")) {
			multiLayerRLManagerPython.reinforcementFeedback(task);
		}
