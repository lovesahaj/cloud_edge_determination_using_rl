...
//initialize method provides a way to configure and initialize the system based on the architecture value
	...
	protected void assignTaskToVm(int vmIndex, Task task) {
		if (vmIndex == -1) { //it implies that there is no available VM to assign the task to.
			// It is not necessary as the SimulationManager in sendFromOrchToDestination() will count this error and notify you
